      ...
      <main className="mx-auto flex min-h-[calc(100vh-8rem)] max-w-7xl items-center justify-center px-4 py-12 sm:px-6 lg:px-8">
        <div className="grid w-full max-w-6xl gap-6 lg:grid-cols-[0.95fr_1.05fr]">
          <motion.div initial={{ opacity: 0, x: -18 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} className="rounded-[2rem] border border-white/70 bg-white/70 p-8 shadow-xl backdrop-blur-xl sm:p-10">
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-gold-600">Welcome back</p>
            <h1 className="mt-3 font-display text-3xl font-semibold text-purple-950 sm:text-4xl">Your next chapter starts here.</h1>
            <p className="mt-4 max-w-lg text-sm leading-7 text-slate-600">Sign in to save pieces, track your orders, and step into a space made for comfort and style.</p>
            <div className="mt-8 space-y-3">
              {['Tailored recommendations', 'Fast checkout', 'Private wishlist'].map((item) => (
          ...
          <motion.form initial={{ opacity: 0, x: 18 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} onSubmit={handleSubmit} className="relative overflow-hidden rounded-[2rem] border border-white/70 bg-white/85 p-7 shadow-2xl backdrop-blur-xl sm:p-8">
            ...
            <div className="relative">
              <p className="text-sm font-semibold uppercase tracking-[0.3em] text-gold-600">Log in</p>
              <h2 className="mt-3 font-display text-3xl font-semibold text-purple-950">Welcome home</h2>
              ...
              <div className="mt-6 space-y-4">
                ...
                <motion.button
                  ...
                  type="submit"
                  disabled={isLoading}
                  className="flex w-full items-center justify-center gap-2 rounded-full bg-purple-700 px-4 py-3 font-semibold text-white shadow-lg shadow-purple-200 disabled:opacity-60"
                >
                  {isLoading ? 'Opening your space…' : 'Continue'}
                  <ArrowRight size={16} />
                </motion.button>
              </div>
              <p className="mt-5 text-sm text-slate-600">
                New to LIVORA? <Link to="/register" className="font-semibold text-purple-700">Create an account</Link>
              </p>
            </div>
          </motion.form>
        </div>
